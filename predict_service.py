...
class Predict:
    ...
    def __init__(self, channel_log: str):
        ...
        write_log(log_queue)
        self.output_path = const.OUTPUTS_DIR
        self.output_path = Path(STORAGE_PATH)
    ...
    def find_latest_lora_weights(self) -> Optional[str]:
        ...
        try:
            ...
            lora_path = self.output_path / "pytorch_lora_weights.safetensors"
            if lora_path.exists():
                return str(lora_path)
            
            # Check in lora_weights directory
            lora_weights_dir = self.output_path / "lora_weights"
            ...
            print(f"Error finding LoRA weights: {e}")
            return None
